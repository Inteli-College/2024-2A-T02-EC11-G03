      ...
      fetch('http://localhost:8000/upload', {
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            throw new Error('Falha no upload.'); // Lança um erro se não for 200
          }
        })
        .then((data) => {
          setLoading(false); // Finaliza o carregamento
          setImageProcessed(`https://demeters3.s3.amazonaws.com/processed/uploads/${file.name}`);
          setImage(URL.createObjectURL(file)); // Mostra a imagem carregada
          setIsModalOpen(true); // Abre o modal após o upload
          setError(null); // Upload bem-sucedido
        })
        .catch((error) => {
          setLoading(false); // Finaliza o carregamento
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-4 rounded shadow-lg">
            ...
            <img src={imageProcessed} alt="Imagem processada" className="h-auto max-w-full mb-4" />
            <button 
